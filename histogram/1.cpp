...
void generateNegativeImage(Mat img, int original_image_histogram_values[])
{
	for (int i = 0; i < img.rows; i++)						//Loop that computes negative greyscale image
	{
		for (int j = 0; j < img.cols; j++)
		{
			...
			original_image_histogram_values[int(img.at<uchar>(i, j))]++;
						//Generates histogram data for negative greyscale image	
		}
	}
}
...
{
	int histogram_height = 400,histogram_width = 512,max_intensity_value = 0;
...
line(histogram_image, Point(binary_width*(i), histogram_height), Point(binary_width*(i), histogram_height - hist[i]), Scalar(0,0,0),1,8,0);
	//line(histogram_image, Point(j, histogram_height-20), Point(j, (histogram_height-20) - original_image_histogram_values[i]), 
...
void equalizeHistogram(Mat img)
{
	...
	int size = img.rows*img.cols,scaled_histogram[256],final_histogram[256];
	float alpha = 255.0 / size,intensity_probability[256],equalized_histogram[256];
	for (int i = 0; i < 256; i++)
	{
